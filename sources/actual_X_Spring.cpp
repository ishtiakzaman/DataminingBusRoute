...
int main()
{
	...
	FILE *fp2 = fopen("joinSpringRouteScheduled.csv", "r");
	FILE *fpWrite = fopen("difference_Spring_X_Route_M-R.csv", "w");

	int fixedRouteID = 364;
	...
	while (fscanf(fp2, " %[^\n]", cline) != EOF)
	{
		...
		if (fixedRouteID == 364)
		{
			// Special cases of X route
			// Stadium (67) -> Stadium (X) (76)
			// IMU (12) -> IMU(X) (64)
			if (fromStop == 67) fromStop = 76;
			if (fromStop == 12) fromStop = 64;			
		}
